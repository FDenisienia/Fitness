...
/**
 * Chat Admin ↔ Coach: consultas y dudas de los coaches con el administrador.
 * Misma interfaz que el chat coach-cliente, pero para comunicar coaches con admin.
 */
export default function AdminCoachChatInbox({ currentUser, conversations = [], singleConversation = null, onUpdate, embedded }) {
  const [selectedConv, setSelectedConv] = useState(singleConversation?.id ?? (conversations[0]?.id ?? null));
  const [messages, setMessages] = useState(store.getAdminCoachChatMessages());
  ...
  useEffect(() => {
    setMessages(store.getAdminCoachChatMessages());
  ...
  const getOtherParticipant = (conv) => {
    if (!conv) return null;
    return users.find(u => u.id === (isAdmin ? conv.coachId : conv.adminId));
  };

  const unreadForCurrent = (conv) => isAdmin ? (conv.adminUnreadCount || 0) : (conv.coachUnreadCount || 0);
  ...
  const sendMessage = (e) => {
    ...
    const updated = [...store.getAdminCoachChatMessages(), newMsg];
    store.setAdminCoachChatMessages(updated);
    setMessages(updated);

    const convsData = store.getAdminCoachConversations();
    const otherUnread = isAdmin ? 'coachUnreadCount' : 'adminUnreadCount';
    ...
    store.setAdminCoachConversations(updConvs);
    onUpdate?.();
  ...
  const markAsRead = () => {
    ...
    const convsData = store.getAdminCoachConversations();
    const key = isAdmin ? 'adminUnreadCount' : 'coachUnreadCount';
  ...
  const pendingReply = isAdmin && (activeConv?.adminUnreadCount || 0) > 0;
      ...
      {(convs.length > 1 || !singleConversation) && (
        <aside className={`chat-sidebar ${showSidebar ? 'chat-sidebar--open' : ''}`}>
          <div className="chat-sidebar-header">
            <h3 className="chat-sidebar-title">{isAdmin ? 'Consultas de coaches' : 'Soporte'}</h3>
          ...
          <div className="chat-conversation-list">
            {convs.length === 0 ? (
              <div className="chat-empty-list">
                {isAdmin ? 'No hay consultas de coaches' : 'Escribe para contactar con soporte'}
              </div>
            ) : (
              convs.map(conv => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="chat-conv-body">
                      <div className="chat-conv-name">{o ? `${o.name} ${o.lastName}` : (isAdmin ? 'Coach' : 'Admin')}</div>
        ...
        {!activeConv ? (
          <div className="chat-welcome">
            ...
            {convs.length === 0 && !isAdmin && (
              <p className="text-muted small">Envíanos tu consulta o duda y te responderemos pronto</p>
            )}
          </div>
        ) : (
          <>
            <header className="chat-header">
              ...
              <div className="chat-header-info">
                <h4 className="chat-header-name">{other ? `${other.name} ${other.lastName}` : 'Chat'}</h4>
                <span className="chat-header-role">{isAdmin ? 'Coach' : 'Soporte / Admin'}</span>
              </div>
