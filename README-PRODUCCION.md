...
| **P3009** / *«migrate found failed migrations»* / no arranca el backend tras un deploy | Una migración quedó **marcada como fallida** en `_prisma_migrations` (a veces por un SQL incompatible o por un deploy interrumpido). Prisma **no aplicará** migraciones nuevas hasta resolverlo. Pasos típicos en Railway (carpeta **`backend`**, mismo `DATABASE_URL` que producción): **1)** Conecta un shell o un servicio one-off con Node. **2)** Si la migración falló a medias, en MySQL elimina tablas huérfanas (solo si existen y están vacías o es aceptable perderlas): `DROP TABLE IF EXISTS client_routine_exercise_sets;` y luego `DROP TABLE IF EXISTS client_routine_exercises;` (**orden:** primero `sets`, luego `exercises`). **3)** Marca la migración como revertida para que pueda reintentarse: `npx prisma migrate resolve --rolled-back 20260506120000_client_routine_instance_sets`. **4)** Vuelve a desplegar (o ejecuta `npx prisma migrate deploy`). Si el error original fue de SQL, corrige el archivo de migración en el repo, haz commit y repite desde el paso 3. Si la migración **sí se aplicó bien** pero quedó mal el registro, usa `--applied` en lugar de `--rolled-back` (solo si comprobaste en MySQL que tablas e índices existen). Documentación: [Migraciones fallidas (Prisma)](https://www.prisma.io/docs/guides/migrate/production-troubleshooting). |
